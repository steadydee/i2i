...
# ---------------------------------------------------------------------
# 3. policy_qna_chain  – Employee-handbook RAG answer
# ---------------------------------------------------------------------
...
_POLICY_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "Answer the user question **only** with information found in the provided excerpts. "
...
def _policy_qna(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ----- Retrieve top-k handbook chunks -------------------------------
    retriever = SupaRetriever("vector_chunks", doc_id=doc_id)
